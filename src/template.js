...
const { IGNORE_PATHS, JPEG_QUALITY, JPEG_PROGRESSIVE, PNG_QUALITY, WEBP_QUALITY, COMPRESS_ONLY, EXTENSION_TO_SHARP_FORMAT_MAPPING } = require('./constants')
...
const generateImage = (images, pr, commit) => {
    return images.map((image) => {
        return {
            ...
            diffUrl:
                commit && pr
                    ? generateDiffUrl(image, pr, commit)
                    : null
        }
    })
}
